...
DEMO_README = 'README_original.md'
...
flds_internals = ['idn', # index of new row in main readme table
                  ...
                  'has_transcript', # bool
                  'status', # save to enable update
                  'notes', # save to enable update
                 ]
...
class TranscriptMeta:
    ...
    def check_naming_keys(self):
        msg = "Warning. "
        msg += "check_naming_keys :: metadata[{}] has no value."
        for k in ['idn','title_kw','presenter']:
            if self.metadata[k].startswith(NA):
                print(msg.format(k))
                return 


    def check_key(self, k):
        if self.metadata[k] == NA:
    ...
    def set_path_keys(self):
        ...
        i = self.metadata['idn']
        transx = F"{i}-{pres_first}-{pres_last}-{tkw}"
        self.metadata['transcript_md'] = transx + '.md'
    ...
    def parse_md(self, idn, year):
        ...
        for i, s in enumerate(md_lines):
            if s.startswith(HDR2):
                ...
                k = kval[0].strip().title()
                if add_key and k in h2_subheaders:
                    if k == 'Video': k = 'video_url'
        ...
        if has_transcript:
            ...
            md_kvals['formatted_transcript'] = md_lines[trans_idx[0]+i:]
        else:
        ...
        tmd = md_kvals.get('transcript_md', NA).strip()
        if tmd == NA:
            md_kvals['meta_json'] = NA
        else:
    ...
    def __repr__(self):
        if self.metadata['has_transcript']:
            ...
            without_trx['formatted_transcript'] = "< NOT SHOWN >"
            msg = "\nThe transcript is excluded from the repr as it "
            msg += "can be VERY long. To view it, use:\n"
            msg += "EventMeta.pp(TranscriptMeta.metadata"
            msg += "['formatted_transcript'])"
            warn(msg)
...
class MarkdownTpl:
    def __init__(self, tpl_dir=DIR_MD_TPL, md_tpl_name=MD_TPL):
        ...
        self.hdrs_to_keys = {'Transcript':'transcript_md',
